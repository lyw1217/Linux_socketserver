/*
 ...
 Description : 멀티플렉싱 가능하게끔 epoll 사용 / 문자열 split기능 추가 // 함수로 구현은 실패..
 ============================================================================
 */
    ...
    while(1) {
        ...
        for (i = 0; i < event_cnt; i++) {
            if (ep_events[i].data.fd == server_sock) {
				...
				/*로그처리루틴*/
				memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
				sprintf(log_Temp,"Server : Connect Request %d, IP : %s ", client_sock, addr_Temp);
				print_log(log_Temp);
            } else {
                ...
                if (str_len == 0) { // close request!
                    ...
                    /*로그처리루틴*/
                    memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
                    sprintf(log_Temp,"Server : Closed client %d, IP : %s", ep_events[i].data.fd, addr_Temp);
                    print_log(log_Temp);
                } else {
                    /*로그처리루틴*/
                    memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
                    sprintf(log_Temp,"Server : Client %d send : %s",ep_events[i].data.fd, buf);
                    print_log(log_Temp);
                    ...
                    if(splitLists[1] != NULL){
						...
						if(!strcmp(splitLists[1], "sign")){
							...
							for(num = 0; num < MAX_CLIENT; num++){
								...
								if(User[num].id != NULL){
									...
									if(!strcmp(splitLists[0], User[num].id)){
										...
										/*로그처리루틴*/
										memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
										sprintf(log_Temp,"Server : %s is already exist", splitLists[0]);
							...
							if(signup_flag){
								...
								for(num = 0; num < strlen(splitLists[0]); num++){
									User[user_count].id[num] = splitLists[0][num];
								}
								for(num = 0; num < strlen(splitLists[2]); num++){
									User[user_count].pw[num] = splitLists[2][num];
								}
							
								/*로그처리루틴*/
								memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
								sprintf(log_Temp,"Server : Client '%s' sign up!", User[user_count].id);
								log_Temp_len = strlen(log_Temp);
								if(write(ep_events[i].data.fd, log_Temp, log_Temp_len) != log_Temp_len){
									error_handling("write() Error!");
								}
							...
							close(epfd);
							return 0;
						}else if(!strcmp(splitLists[1], "ALLMSG")){
							/*로그처리루틴*/
							memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
							sprintf(log_Temp,"[%s]%s", splitLists[0], buf);
							log_Temp_len = strlen(log_Temp);
							print_log(log_Temp);
							for(num = 0; num < event_cnt; num++){
								if(write(ep_events[i].data.fd, log_Temp, log_Temp_len) != log_Temp_len){
									error_handling("write() Error!");
								}
							}
...
#ifdef DEBUG
										printf("[not sign] user '%d' id %s / pw %s\n", num, User[num].id, User[num].pw);
										printf("user_count = %d\n", user_count);
#endif
...
void print_log(char *buff){
	...
	sprintf(log_buff, "%02d-%02d-%02d %02d:%02d:%02d %s\n",tm.tm_year-100, tm.tm_mon+1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, buff); // 메세지 저장
