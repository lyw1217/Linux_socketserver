        ...
        for (i = 0; i < event_cnt; i++) {
            if (ep_events[i].data.fd == server_sock) {
                ...
                if (str_len == 0) { // close request!
                    ...
                    while(token){
                    	strcpy(splitLists[split_count], token);
                    ...
                    if(!strcmp(splitLists[1], "sign")){
                    	...
                    	for(int num = 0; num < MAX_CLIENT; num++){
                    		if(!strcmp(splitLists[0], User[num].id)){
                    			...
                    			/*로그처리루틴*/
                    			memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
                    			sprintf(log_Temp,"Server : %s is already exist\n", splitLists[0]);
                    			write(ep_events[i].data.fd, log_Temp, sizeof(log_Temp));
                    			print_log(log_Temp);
                    			break;
                    		}
                    		// 마지막까지 중복된 ID가 없다면 플래그
                    		if(num == (MAX_CLIENT-1)) signup_flag = 1;
                    	}
                    	if(signup_flag){
                    		// 중복된 id 없으면 구조체에 등록
                    		...
                    		/*로그처리루틴*/
                    		memset(log_Temp, 0x00, sizeof(log_Temp)); // 버퍼초기화
                    		sprintf(log_Temp,"Server : Client '%s' sign up!\n", User[user_count].id);
                    		write(ep_events[i].data.fd, log_Temp, sizeof(log_Temp));
                    		print_log(log_Temp);
